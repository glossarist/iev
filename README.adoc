...
[WARNING]
--
As of 2026-02-10, Electropedia is limiting programmatic access to its URLs, and is blocking access from GitHub Actions completely.
The gem has had its headers updated so that local programmatic access is reenabled, but can not be relied on as a means of
programmatically accessing Electropedia content.
--
